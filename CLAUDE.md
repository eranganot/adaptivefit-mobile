...
**Repo location: `C:\dev\adaptivefit-mobile`** (moved off OneDrive 2026-06-22 — the old `…\Eran's dev\…\adaptivefit-mobile` path is retired; delete it once a deploy from here is confirmed).
...
- **The sandbox cannot push** (no git credentials). Make the edits, then hand Eran one copy-paste PowerShell block to run on his machine. Never try to `git push` from the sandbox.
- **Deploy via the `ship-it` skill** — it has the exact git + Railway recipe.
- **Triage bugs via the `app-bug-triage` skill** — get the Railway log line / deploy status before editing.

## Known sharp edges
- After renaming/deleting a dynamic route, nuke `.next` or typecheck references the dead route. If a route dir won't delete, use `git rm -rf`. (Now on a plain local path, the old OneDrive `.git/index.lock` failures should be gone.)
