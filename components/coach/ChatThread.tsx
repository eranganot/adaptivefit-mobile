...
export function ChatThread({ workoutLogId, initialMessages, initialActions }: ChatThreadProps) {
  ...
  return (
    // The chat takes the full available width but pins the input to the
    // bottom of the viewport using `fixed` positioning so it sits ABOVE the
    // app shell's bottom nav (which itself is fixed at bottom-0). Messages
    // scroll within the page; we add bottom padding to the messages area so
    // the last message isn't hidden behind the input bar.
    // Direction flips for Hebrew so bubbles mirror.
    <div
      dir={isRtl ? "rtl" : "ltr"}
    ...
    >
      ...
      {/* Messages — pb-44 leaves room for the fixed input bar above the bottom nav */}
      <div className="flex-1 px-4 py-2 space-y-3 pb-44">
        {messages.length === 0 && (
      ...
      {/* Input — FIXED to the viewport bottom, pinned ABOVE the parent app
          shell's bottom nav (which itself is fixed bottom-0). The bottom-20
          offset (5rem) clears the bottom nav reliably across mobile Chrome.
          Textarea auto-grows up to ~5 lines. dir="auto" flips alignment per
          content language. */}
      <div className="fixed bottom-20 left-1/2 -translate-x-1/2 w-full max-w-md px-4 pb-2 pt-3 bg-gray-50 dark:bg-slate-950 border-t border-gray-200 dark:border-slate-800 z-20">
